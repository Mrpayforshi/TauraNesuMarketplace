...
//
// The whole handler is wrapped in try/catch below. Previously, anything
// that threw rather than returning a Supabase { error } object (a thrown
// exception from createServerClient(), a network blip talking to
// Supabase, etc.) would produce Next's generic framework error response
// instead of this route's normal JSON shape. The frontend (src/app/sell/
// page.tsx) does `const data = await res.json()` unconditionally on a
// non-ok response — if that body isn't valid JSON, .json() itself throws,
// which is caught by the page's outer catch and shown as a generic
// "Network error" message. That makes a real server-side failure here
// indistinguishable from an actual network problem on the user's end.
// Wrapping in try/catch guarantees a parseable { error } body either way,
// and the console.error gives us something to actually look at the next
// time a submission fails.
...
export async function POST(request: NextRequest) {
  try {
    ...
    // talking to Supabase, an unexpected exception anywhere above.
    console.error('[POST /api/submissions] Unhandled exception:', e)
